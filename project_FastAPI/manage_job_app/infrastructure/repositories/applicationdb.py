...
class ApplicationRepository(IApplicationRepository):
    ...
    async def get_all_applications(self) -> Iterable[Any]:
        ...
        query = (
            select(application_table, offer_table, user_author_alias, user_applicant_alias)
            .select_from(
                join(
                    application_table,
                    offer_table,
                    ...
                    user_author_alias,
                    offer_table.c.author_id == user_author_alias.c.id
                ).join(
                    user_applicant_alias,
                    application_table.c.user_id == user_applicant_alias.c.id
                )
            )
    ...
    async def get_by_id(self, application_id: int) -> Any | None:
        ...
        query = (
            select(application_table, offer_table, user_table)
            .select_from(
                join(
                    application_table,
                    offer_table,
                    ...
                    user_table,
                    application_table.c.user_id == user_table.c.id
                )
            )
    ...
    async def add_application(self, data: ApplicationIn) -> Any | None:

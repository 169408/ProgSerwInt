...
class UserRepository(IUserRepository):
    ...
    async def get_by_id(self, user_id: UUID5) -> Any | None:
        ...
        """
        employer_query = select(employer_table).where(employer_table.c.id == user_id)
        employee_query = select(employee_table).where(employee_table.c.id == user_id)

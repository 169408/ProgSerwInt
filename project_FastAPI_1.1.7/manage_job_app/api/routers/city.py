...
async def create_city(
    ...
    if authorized_user.email != "gozuhost@gmail.com":
        raise HTTPException(status_code=403, detail="Forbidden. You haven't role HOST to create city")
...
@inject
async def get_city_by_continent(
    country_id: int,
    service: ICityService = Depends(Provide[Container.city_service]),
) -> Iterable:
...
async def update_city(
    ...
    authorized_user = await user_service.get_by_id(user_uuid)
    if authorized_user.email != "gozuhost@gmail.com":
        raise HTTPException(status_code=403, detail="Forbidden. You haven't role HOST to update city data")
...
async def delete_city(
    ...
    authorized_user = await user_service.get_by_id(user_uuid)
    if authorized_user.email != "gozuhost@gmail.com":
        raise HTTPException(status_code=403, detail="Forbidden. You haven't role HOST to delete city from db")
